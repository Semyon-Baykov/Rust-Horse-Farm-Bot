...
def bhelp():
    ...
    output.add_field(name='produce [scrap amount] [# of horses]',
                     value='Calculates the amount of dung required to produce that scrap, the amount of time it will compost, and how long it will take to sell the fertilizer.',
                     inline=False)
    output.add_field(name='fts [# of fertilizer]',
                     value='(Fertilizer to scrap) - Calculates the amount of scrap you will get for a given amount of fertilizer.',
                     inline=False)
    output.add_field(name='stf [# of scrap]',
                     value='(Scrap to fertilizer) - Calculates the amount of fertilizer you will need for a given amount of scrap',
                     inline=False)
    return output
...
def fertToScrap(fert):
    output = discord.Embed(
        ...
        description=f'On a sale cap server, this will take {round((fert - 68) / 27, 2)} minutes to sell.',
        color=discord.Color.dark_green())
    return output
...
def scrapToFert(scrap):
    output = discord.Embed(
        ...
        description=f'On a sale cap server, this will take {round((((scrap / 3) * 2) - 68) / 27, 2)} minutes to sell.',
        color=discord.Color.dark_green())
    return output
